    ...
    <main className="min-h-screen bg-[#fbfbfd] flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center pt-24 pb-12 px-4">
        <div className="apple-glass rounded-[2rem] border border-black/5 w-full max-w-md" style={{ padding: "48px" }}>
          <h1 className="text-3xl font-bold mb-2">Phone Login</h1>
          ...
          <div className="space-y-4">
            ...
            {!confirmation ? (
              ...
              <>
                ...
                <button onClick={verifyOtp} disabled={loading} className="w-full btn-primary py-3">{loading ? "Verifying..." : "Verify & Login"}</button>
              </>
            )}
            <div id="recaptcha-container" />
          </div>
          <div className="mt-4 text-sm">New user? <Link href="/register" className="text-blue-600">Register</Link></div>
        </div>
      </div>
      <Footer />
    </main>
